...
(define circle-canvas%
  (class canvas%
    ...
    (define/public (draw-circles {closest : circle} (others-without-closest : (U False [Listof circle]) #f)) : Void
      (define dc : (Instance DC<%>) (send (cast this (Instance Canvas%)) get-dc))
      (send dc clear)
      (for ((c : [Listof circle] (or others-without-closest *circles))) (draw-1-circle dc white-brush c))
      (when closest (draw-1-circle dc solid-gray closest)))
    
    (define (paint-callback {_self : Any} {_evt : Any}) : Void 
      (cond
